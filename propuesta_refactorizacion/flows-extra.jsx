...
function NotificationsPanel({ onClose, role }) {
  const items = {
    admin: [
      { kind: 'danger', t: 'Hot USDT al 18%', d: 'Wallet corporativa cerca del piso operativo · acción requerida', when: '12 min', read: false },
      { kind: 'warn',   t: 'Disputa P2P #4821', d: 'SLA vence en 4 min · sin asignar', when: '38 min', read: false },
      { kind: 'info',   t: '2ª aprobación pendiente', d: 'Sergio R. firmó la distribución a 41 wallets', when: '1 h', read: false },
      { kind: 'success',t: 'Compliance al día', d: 'Cola del turno cerrada con 96.4% SLA', when: '3 h', read: true },
      { kind: 'info',   t: 'Backup semanal completado', d: 'Snapshot de datos blockchain guardado', when: '8 h', read: true },
    ],
    operator: [
      { kind: 'warn',  t: 'AML alert · Mateo F.', d: 'Asignado a vos · SLA 27 min', when: '6 min', read: false },
      { kind: 'info',  t: 'Nuevo KYC L2', d: 'Lucía González pidió revisión', when: '24 min', read: false },
      { kind: 'success',t: 'Caso resuelto', d: 'KYC de Sofía Pérez aprobado', when: '1 h', read: true },
    ],
    user: [
      { kind: 'success',t: 'Recibiste 120 cUSD', d: 'De Sofía Pérez · "Cena 🍕"', when: '12 min', read: false },
      { kind: 'info',  t: 'Tu compra P2P se confirmó', d: '500 USDT acreditados', when: '2 h', read: false },
      { kind: 'warn',  t: 'Verificación L3 recomendada', d: 'Estás cerca de tu límite mensual', when: '1 d', read: true },
    ],
  }[role || 'admin'];
  ...
  return (
    ...
    >
      <div style={{ padding: '12px 14px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <a href="#" style={{ fontSize: 11.5, color: 'var(--accent-text)', fontWeight: 500 }}>Marcar todas leídas</a>
      </div>
      <div style={{ overflowY: 'auto', flex: 1 }}>
        {items.map((it, i) => (
          <div key={i} style={{ padding: '10px 14px', borderBottom: i < items.length - 1 ? '1px solid var(--border)' : 0, display: 'flex', gap: 10, background: it.read ? 'transparent' : 'var(--surface)', cursor: 'default', position: 'relative' }}>
      ...
      <div style={{ padding: '8px 14px', borderTop: '1px solid var(--border)', textAlign: 'center' }}>
        <a href="#" style={{ fontSize: 11.5, color: 'var(--accent-text)', fontWeight: 500 }}>Ver todas las notificaciones →</a>
      </div>
    </div>
  );
}
