...
const ROUTES = {
  admin: {
    ...
    permissions:() => <PermissionsScreen role="admin" />,
...
function InteractiveApp() {
  ...
  const onFlowComplete = (type) => {
    ...
    if (type === 'wallet-create')   toast('Wallet creada · lista para recibir fondos', 'success');
    if (type === 'permissions')     toast('Permisos actualizados · registrado en auditoría', 'success');
    if (type === 'create-user')     toast('Usuario creado · invitación enviada', 'success');
    if (type === '2fa-setup')       toast('2FA activado · guardá tus códigos de respaldo', 'success');
  };
  ...
  return (
    <NavCtx.Provider value={navContextValue}>
      ...
      {mobileOpen && window.MobileApp && <window.MobileApp onClose={() => setMobileOpen(false)} />}
...
function RoleSwitcher({ role, onChange, onLogout, onMobile }) {
  ...
  return (
    <div style={{
      ...
      <button
        onClick={onMobile}
        title="Ver versión móvil interactiva"
        style={{
