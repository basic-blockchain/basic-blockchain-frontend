                ...
                {[
                  ...
                  <tr key={i} style={{ cursor: 'pointer' }}
                      onClick={() => nav.openFlow('tx-detail', {
                        tx: { id: 'tx_d' + i, sender: r[3], receiver: r[0].toLowerCase().includes('withdraw') ? 'external' : 'platform', amount: r[1].split(' ')[0], currency: r[1].split(' ')[1] || 'USDT', fee: '0.0001', age: '—', size: 320 },
                        status: 'completed', block: 11 - i, confirmedAt: '2026-05-14 01:47',
                      })}>
              ...
              <table className="tbl" style={{ fontSize: 12 }}>
                ...
                <tbody>
                  {[
                    ...
                    <tr key={i} style={{ cursor: 'pointer' }}
                        onClick={() => nav.openFlow('send-detail', {
                          row: {
                            type: 'Tesorería', from: r[3].split('→')[0].trim(), to: r[3].split('→')[1].trim(),
                            asset: r[1], amount: r[2], ref: 'op_' + Math.random().toString(36).slice(2, 9),
                            status: 'completed', when: r[5], conf: r[4],
                          },
                        })}>
                      <td style={{ fontWeight: 500 }}>{r[0]}</td>
                      <td className="mono">{r[1]}</td>
