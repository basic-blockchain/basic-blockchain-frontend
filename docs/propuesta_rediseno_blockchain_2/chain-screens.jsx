              ...
              <tbody>
                {PEERS.map((p, i) => (
                  <tr key={i} style={{ cursor: 'pointer' }}
                      onClick={() => nav.openFlow('peer-actions', { peer: p })}>
                    ...
                    <td onClick={e => e.stopPropagation()}>
                      <button className="btn btn-sm btn-icon btn-ghost" onClick={() => nav.openFlow('peer-actions', { peer: p })}>{I.more}</button>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      <div className="main">
        ...
        <div className="page">
          <div className="scr-h">
            ...
            <button className="btn btn-sm btn-primary" onClick={() => nav.openFlow('validation-run', {})}>{I.refresh}<span>Re-validar cadena</span></button>
          </div>
      ...
      <div className="main">
        ...
        <div className="page">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: 12 }}>
            ...
            <div>
              {window.HealthLogsLive ? <window.HealthLogsLive /> : (
                <>
                  <div className="section-h">Logs · últimos eventos</div>
                  <div className="card" style={{ padding: 0 }}></div>
                </>
              )}
            </div>
          </div>
...
function CurrencyCatalogScreen({ role = 'admin' }) {
  const nav = useNav();
  const [form, setForm] = useSc({ code: '', name: '', decimals: 8, active: true });
  return (
    <div className="scr">
        ...
        <div className="page">
          <div className="scr-h">
            ...
            <button className="btn btn-sm btn-primary"
              onClick={() => nav.openFlow('currency-form', {})}>
              {I.plus}<span>Agregar moneda</span>
            </button>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 14 }}>
                ...
                <tbody>
                  {[
                    ...
                    <tr key={code} style={{ cursor: 'pointer' }}
                        onClick={() => nav.openFlow('currency-form', { code, name, decimals: dec, type, active: true })}>
                      ...
                      <td><button className="btn btn-sm btn-icon btn-ghost" onClick={(e) => { e.stopPropagation(); nav.openFlow('currency-form', { code, name, decimals: dec, type, active: true }); }}>{I.edit}</button></td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div>
              <div className="card" style={{ padding: 14 }}>
                <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 4 }}>Crear rápido</div>
                <div className="muted" style={{ fontSize: 11.5, marginBottom: 14 }}>Para flujos completos usá "Agregar moneda".</div>

                <div className="fld"><label>Code</label><input className="mono" placeholder="NATIVE" value={form.code} onChange={e => setForm({...form, code: e.target.value.toUpperCase()})} /></div>
                <div className="fld"><label>Nombre</label><input placeholder="Native" value={form.name} onChange={e => setForm({...form, name: e.target.value})} /></div>
                ...
                <button className="btn btn-primary" style={{ width: '100%', justifyContent: 'center' }}
                  ...
                  onClick={() => { nav.toast('Moneda ' + form.code + ' creada · activa en el catálogo', 'success'); setForm({ code: '', name: '', decimals: 8, active: true }); }}>
                  Crear
                </button>
              </div>
            </div>
        ...
        <div className="page">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 14 }}>
            ...
            <div className="card" style={{ padding: 16 }}>
              ...
              <button className="btn btn-primary" style={{ width: '100%', justifyContent: 'center' }}
                onClick={() => nav.toast('Tasa manual NATIVE → USDT guardada (1.05)', 'success')}>
                Guardar tasa
              </button>
            </div>
          </div>
