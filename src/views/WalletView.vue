...
async function handleCreateWallet() {
  ...
  try {
    ...
    toast.add({ severity: 'error', summary: 'Wallet creation failed', detail: e instanceof Error ? e.message : 'Unexpected error', life: 4000 })
  } finally {
...
async function onSeedConfirmed() {
  ...
  toast.add({ severity: 'success', summary: 'Wallet created', detail: 'Your new wallet is ready', life: 3000 })
}
...
async function submitTransfer() {
  ...
  if (!isValidMnemonic(mnemonic)) {
    toast.add({ severity: 'warn', summary: 'Invalid phrase', detail: 'Check your recovery phrase', life: 3000 })
    return
  }
  if (Number(amount) <= 0) {
    toast.add({ severity: 'warn', summary: 'Invalid amount', detail: 'Amount must be positive', life: 3000 })
    return
  }
  ...
  try {
    ...
    toast.add({ severity: 'success', summary: 'Transfer submitted', detail: 'Transaction is in the mempool — mine a block to confirm it', life: 5000 })
    ...
    >
      ...
      >
        ...
        <div class="wallet-balance">
          {{ w.balance.toFixed(8) }} <span class="currency">NATIVE</span>
        </div>
        <div class="wallet-footer">
          <span
            class="wallet-pub"
            :title="w.public_key"
          >{{ w.public_key.slice(0, 20) }}…</span>
        </div>
      </div>
    </div>
    ...
    >
      ...
      >
          ...
          <div class="form-field">
            ...
            >
              ...
              <option
                ...
                :value="w.wallet_id"
              >
                {{ w.wallet_id.slice(0, 16) }}… ({{ w.balance.toFixed(4) }})
              </option>
            </select>
          </div>
          <div class="form-field">
        ...
        <div class="form-field full-width">
          ...
          <span
            v-if="!mnemonicValid"
            class="field-error"
          >Invalid recovery phrase</span>
          <span
            v-else
            class="field-hint"
          >Your phrase never leaves the browser — it is used only to sign the transaction locally</span>
        </div>
      ...
      <p class="transfer-note">
        <span
          ...
          aria-hidden="true"
        />
        The transfer is placed in the mempool. Ask the operator to mine a block to confirm it.
        The balance shown above updates once a block is mined.
      </p>
    </section>
  </div>
</template>
...
/* Wallet cards */
.wallet-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 1rem; margin-bottom: 2rem; }
.wallet-card {
...
.wallet-card.frozen { border-color: #f59e0b; opacity: 0.8; }
.wallet-card-header { display: flex; align-items: center; justify-content: space-between; margin-bottom: 0.75rem; }
.wallet-id { font-family: monospace; font-size: 0.82rem; color: var(--text-muted); }
.frozen-badge { font-size: 0.75rem; color: #f59e0b; font-weight: 600; display: flex; align-items: center; gap: 0.2rem; }
.wallet-balance { font-size: 1.5rem; font-weight: 700; color: var(--text-heading); margin-bottom: 0.75rem; }
.currency { font-size: 0.75rem; color: var(--text-muted); font-weight: 400; }
.wallet-footer { font-family: monospace; font-size: 0.75rem; color: var(--text-muted); }
.wallet-pub { white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
...
.transfer-form { display: flex; flex-direction: column; gap: 0.75rem; }
...
.mnemonic-input { resize: vertical; font-family: monospace; }
