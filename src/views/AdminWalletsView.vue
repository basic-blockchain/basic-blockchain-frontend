...
<template>
  <div class="admin-wallets">
    ...
    <div class="filters">
      <input v-model="filterUser" placeholder="Filter by user…" class="filter-input" />
    ...
    <table v-else class="wallets-table">
      ...
      <tbody>
        <tr v-for="w in filtered" :key="w.wallet_id" :class="{ 'row-frozen': w.frozen }">
          <td class="mono">
          ...
          <td>
            <span :class="w.frozen ? 'badge-frozen' : 'badge-active'" class="status-badge">
        ...
        <tr v-if="filtered.length === 0 && !loading">
          <td colspan="7" class="empty">No wallets found.</td>
        </tr>
      </tbody>
    </table>
  </div>
