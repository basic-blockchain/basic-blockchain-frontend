...
const approvers = [
  { name: 'María Acosta', email: 'admin@cadena.io', me: true, get status() { return 'signed' }, when: 'hace 8 s' },
  { name: 'Sergio Romero', email: 'sergio@cadena.io', me: false, get status() { return approver2Signed.value ? 'signed' : 'pending' }, get when() { return approver2Signed.value ? 'hace 2 s' : 'en línea · pendiente' } },
      ...
      <div class="modal-b">
        ...
        <template v-else-if="step === 2 || step === 3">
          ...
          <template v-if="step === 3">
            <div style="font-size:11px;color:var(--text-3);display:flex;justify-content:space-between;margin-bottom:4px">
...
.status-icon.waiting { background: var(--warning-soft); color: var(--warning); }
.status-icon.signed  { background: var(--success-soft); color: var(--success); }
.approver-row {
  display: flex;
  align-items: center;
  gap: 12px;
