...
const chat = [
  { who: 'sys', msg: `Operación P2P-${props.data.opId} iniciada · ${props.data.amount} ${props.data.asset}`, t: '17:42' },
  ...
  <div v-if="submitted" class="modal-scrim" @click.self="emit('close')">
    <div class="modal" style="width:460px">
      ...
      <div class="modal-f">
        ...
        <button class="btn btn-primary" @click="emit('complete'); emit('close')">Siguiente disputa</button>
      </div>
    </div>
  </div>
    ...
    <div class="modal" style="width:760px;max-height:88vh">
      <div class="modal-h" style="padding-bottom:14px">
        <div style="display:flex;align-items:flex-start;justify-content:space-between">
          <div>
            ...
            <p style="margin:0">{{ data.amount }} {{ data.asset }} · vía Mercado Pago · abierta hace 3h 56m</p>
          </div>
      ...
      <div class="modal-b" style="padding-top:8px">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
          <div v-for="(party, pk) in [
            { role: 'Comprador', name: data.buyer, stats: 'L2 · 142 ops · 98.4%', claim: `Reclamó que no recibió los ${data.asset}. Adjuntó comprobante de transferencia bancaria.` },
            { role: 'Vendedor', name: data.seller, stats: 'L3 · 412 ops · 96.1%', claim: 'Declara que nunca recibió el pago. Adjuntó captura de su cuenta bancaria.' },
          ]" :key="pk" class="flow-card" style="padding:12px">
            ...
            <div style="font-size:12px;color:var(--text-2);margin-top:8px">{{ party.claim }}</div>
          </div>
        </div>
        ...
        <div style="display:grid;grid-template-columns:1.4fr 1fr;gap:12px">
          ...
          <div class="flow-card" style="padding:12px">
            ...
            >
              <div class="chat-bubble" :class="m.who">
                ...
                <span v-if="m.who === 'sys'" class="muted" style="font-size:10.5px">{{ m.msg }}</span>
          ...
          <div>
            ...
            <div v-for="ev in [
              { ext: 'PDF', name: 'Comprobante Brubank.pdf', from: 'comprador', size: '124 KB', color: 'var(--accent-text)' },
              { ext: 'JPG', name: 'Saldo cuenta seller.jpg', from: 'vendedor', size: '312 KB', color: 'var(--info)' },
            ]" :key="ev.name" class="flow-card" style="padding:10px;margin-bottom:8px">
              <div style="display:flex;align-items:center;gap:8px">
                <div class="ev-thumb" :style="{ color: ev.color }">{{ ev.ext }}</div>
                <div style="flex:1;min-width:0">
                  <div style="font-size:12px;font-weight:500;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">{{ ev.name }}</div>
                  <div class="muted" style="font-size:10.5px">de {{ ev.from }} · {{ ev.size }}</div>
                </div>
            ...
            <div class="flow-card" style="padding:10px">
              <div
                ...
                :key="label"
                class="check-row"
                :style="{ borderBottom: i < autoChecks.length - 1 ? '1px solid var(--border)' : 'none' }"
              >
            ...
            <div style="margin-top:12px">
              <label style="font-size:11px;font-weight:500;color:var(--text-2);display:block;margin-bottom:5px">
...
.chat-row.seller { justify-content: flex-end; }
...
.chat-bubble.buyer { background: var(--surface-2); }
.chat-bubble.seller { background: var(--info-soft); }
.chat-bubble.sys { background: transparent; border-radius: 999px; }
.chat-who { font-size: 10px; color: var(--text-3); margin-bottom: 1px; font-weight: 600; }
.ev-thumb {
  width: 36px;
  height: 44px;
