...
const riskColor = (r: string) =>
  r === 'Crítico' ? 'var(--danger)' : r === 'Alto' ? 'var(--warning)' : r === 'Bajo' ? 'var(--success)' : 'var(--info)'
const riskBg = (r: string) =>
  r === 'Crítico' ? 'var(--danger-soft)' : r === 'Alto' ? 'var(--warning-soft)' : r === 'Bajo' ? 'var(--success-soft)' : 'var(--info-soft)'
...
<template>
    ...
    <div class="modal" style="width:420px">
      <div class="modal-b">
        <div style="text-align:center;padding:20px 0">
          <div
            class="result-circle"
            :style="{ background: decision === 'reject' ? 'var(--danger-soft)' : 'var(--success-soft)', color: decision === 'reject' ? 'var(--danger)' : 'var(--success)' }"
          >
          ...
          <div class="muted" style="font-size:12.5px">Resuelto en 1m 24s · siguiente caso disponible en cola.</div>
        </div>
      </div>
      <div class="modal-f">
        <button class="btn" @click="emit('close')">Cerrar</button>
        <button class="btn btn-primary" @click="emit('complete'); emit('close')">Siguiente caso</button>
      </div>
    </div>
  </div>
  ...
  <div v-else class="modal-scrim" @click.self="emit('close')">
    <div class="modal" style="width:720px;max-height:88vh">
      <div class="modal-h" style="display:flex;align-items:flex-start;justify-content:space-between;padding-bottom:14px">
        <div style="flex:1">
          <div style="display:flex;align-items:center;gap:10px;margin-bottom:6px">
            <div class="user-avatar">{{ data.user[0] }}</div>
            <div>
              <div style="font-size:16px;font-weight:600">{{ data.user }} <span style="font-size:14px;font-weight:400">{{ data.country }}</span></div>
          ...
          <div style="display:flex;gap:6px;align-items:center;font-size:11.5px">
            <span class="bdg" :style="{ background: riskBg(data.risk), color: riskColor(data.risk), border: 0 }">
              Riesgo {{ data.risk }}
            </span>
      ...
      <div class="modal-b" style="padding-top:4px">
        <div style="display:grid;grid-template-columns:1.2fr 1fr;gap:14px">
          ...
          <div>
            ...
            <div class="id-card">
              <div style="font-size:9px;font-weight:500;letter-spacing:0.1em;opacity:0.7">REPÚBLICA ARGENTINA</div>
              <div style="font-size:11px;font-weight:600;margin-top:2px">DOCUMENTO NACIONAL DE IDENTIDAD</div>
              <div style="display:flex;gap:12px;margin-top:14px">
                ...
                <div style="font-size:10.5px;line-height:1.7">
                  ...
                  <div style="font-weight:600;font-size:12px;margin-bottom:4px">{{ lastName }}</div>
                  <div><span style="opacity:0.6">Nombre</span></div>
                  <div style="font-weight:600;font-size:12px">{{ firstName }}</div>
                </div>
              </div>
              <div class="mono" style="position:absolute;bottom:12px;left:16px;font-size:11px;letter-spacing:0.04em">34.281.749</div>
            </div>

            <div style="display:flex;gap:6px;margin-top:8px">
              <button v-for="label in ['Ver', 'Frente', 'Reverso', 'Selfie']" :key="label" class="btn btn-sm" style="flex:1;justify-content:center">
          ...
          <div>
            ...
            <div style="margin-top:14px">
              <label style="font-size:11px;font-weight:500;color:var(--text-2);display:block;margin-bottom:5px">Nota interna</label>
              <textarea
                v-model="note"
      ...
      <div class="modal-f" style="flex-wrap:wrap;gap:6px">
        <button class="btn btn-danger" @click="submit('reject')">
          <span class="pi pi-times" style="font-size:12px" />
          Rechazar
        </button>
        <button class="btn" @click="submit('request')">
          <span class="pi pi-refresh" style="font-size:12px" />
          Pedir reenvío
        </button>
        <div style="flex:1" />
        <button class="btn" @click="emit('close')">Cancelar</button>
        <button class="btn btn-primary" @click="submit('approve')">
