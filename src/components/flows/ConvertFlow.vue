...
const RATES: Record<string, number> = {
  'USDT-BTC': 1 / 67500.20, 'BTC-USDT': 67500.20,
  'USDT-ETH': 1 / 3450, 'ETH-USDT': 3450,
  'USDT-USDC': 1, 'USDC-USDT': 1,
  'BTC-ETH': 67500.20 / 3450, 'ETH-BTC': 3450 / 67500.20,
  'USDT-SOL': 1 / 165, 'SOL-USDT': 165,
  'USDT-cUSD': 1, 'cUSD-USDT': 1,
}
    ...
    <div class="modal" style="width:420px">
      <div class="modal-b">
        <div style="text-align:center;padding:24px 0 8px">
          ...
          <div class="mono" style="font-size:22px;font-weight:600;letter-spacing:-0.01em">+{{ outAmount }} {{ to }}</div>
          <div class="muted" style="font-size:13px">Conversión instantánea desde {{ amount }} {{ from }}</div>
        </div>
      ...
      <div class="modal-f">
        ...
        <button class="btn btn-primary" @click="emit('complete'); emit('close')">Ir a {{ to }}</button>
      </div>
    </div>
  </div>
    ...
    <div class="modal" style="width:440px">
      ...
      <div class="modal-b">
        ...
        <div class="convert-card">
          <div class="muted" style="font-size:10.5px;text-transform:uppercase;letter-spacing:0.04em;margin-bottom:6px">Desde</div>
          ...
          <div class="muted" style="font-size:11px;margin-top:4px">
            ...
            <a href="#" style="color:var(--accent-text)" @click.prevent="amount = '24420.50'">Usar máx</a>
          </div>
        </div>
        ...
        <!-- To card -->
        <div class="convert-card">
          <div class="muted" style="font-size:10.5px;text-transform:uppercase;letter-spacing:0.04em;margin-bottom:6px">A</div>
          <div style="display:flex;align-items:center;gap:10px">
            <select v-model="to" class="asset-select">
              <option v-for="a in toAssets" :key="a" :value="a">{{ a }}</option>
            </select>
            <div class="mono amount-input" style="color:var(--text);background:transparent">{{ outAmount }}</div>
          </div>
          <div class="muted" style="font-size:11px;margin-top:4px">≈ ${{ parseFloat(amount || '0').toFixed(2) }} USD</div>
        </div>

        <div class="dry-run">
          <span class="pi pi-info-circle" style="font-size:13px" />
          <span>Tipo de cambio:</span>
          <span class="mono" style="color:var(--text);font-weight:600">1 {{ from }} = {{ rateLabel }} {{ to }}</span>
        </div>
      </div>
      <div class="modal-f">
        <button class="btn" @click="emit('close')">Cancelar</button>
        <button class="btn btn-primary" @click="executed = true">Convertir {{ amount }} {{ from }}</button>
      </div>
    </div>
  </div>
</template>
...
.swap-btn:hover { background: var(--hover); color: var(--text); }
.detail-row {
  display: flex;
  align-items: center;
