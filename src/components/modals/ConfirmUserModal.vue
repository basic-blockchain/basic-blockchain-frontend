...
<template>
  <div class="modal-scrim" @click.self="emit('close')">
    <div class="modal" role="dialog">
      ...
      <div class="modal-b">
        <div v-if="config.danger" class="warn-box">
          <span class="pi pi-exclamation-triangle" style="font-size:14px;flex-shrink:0;margin-top:1px" />
          <div>
            <div style="font-weight:600;margin-bottom:2px">Acción de alto impacto</div>
        ...
        <div v-if="config.requireReason" class="fld">
          <label>Motivo (requerido)</label>
          <select :value="selectedOpt" @change="onSelectReason(($event.target as HTMLSelectElement).value)">
            <option value="" disabled>Seleccionar motivo…</option>
            <option v-for="opt in config.reasonOpts" :key="opt" :value="opt">{{ opt }}</option>
          </select>
          <textarea
            v-if="isOther"
            v-model="reason"
        ...
        <div v-if="config.hasScope" class="fld">
          <label>Alcance</label>
        ...
        <div v-if="config.requireType" class="fld">
          <label>Para confirmar, escribí el email del usuario: <span class="mono" style="color:var(--text)">{{ user.email }}</span></label>
          <input v-model="confirmText" :placeholder="user.email" autocomplete="off" autofocus />
        </div>
      ...
      <div class="modal-f">
        ...
        <button
          :class="config.danger ? 'btn btn-danger' : 'btn btn-primary'"
          :disabled="!canConfirm"
          @click="submit"
        >
          {{ config.cta }}
